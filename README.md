...
class Rim(arcade.SpriteSolidColor):
    def __init__(self, width, height, x, y, c):
        super().__init__(width, height, x, y, c)
```
to the bottom of the file, right before `main`
...
```python
        ...
        self.radius = radius
```
to the bottom of `Ball.__init__`

# 6 Modify v0 and theta
...
```python
        ...
        self.scored = False
```
to the bottom of Court's `__init__`
